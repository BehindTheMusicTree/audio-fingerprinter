...
def load_required_str_env_var(var_name: str, must_print_value: bool = True) -> str:
    ...
    if must_print_value:
        print_flask(f"{var_name}: {var_value}")
    else:
        print_flask(f"{var_name} is set.")
    return var_value
...
def load_required_path_env_var(var_name: str, must_print_value: bool = True) -> Path:
    ...
    print_flask(f"The path {path} exists on the system.")
    return path
...
def load_calculated_env_paths(base_dir: Path):
    ...
    try:
        ...
        print_flask(f"Error while generating the paths env file: {e.stderr}")  # type: ignore
        raise EnvironmentError(f"Error while generating the paths env file: {e}") from e
...
def load_env_vars_from_file_if_exists(env_file_path: Path):
    if not env_file_path.exists():
        print_flask(f"No env file at {env_file_path}")
    else:
        print_flask(f"Env file provided at {env_file_path} . Loading...")
        dotenv.load_dotenv(env_file_path)
        print_flask("Env file loaded.")
